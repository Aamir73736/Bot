...
def send_main_buttons(chat_id):
    ...
    markup.add(KeyboardButton("ATTACK"), KeyboardButton("Start fuck gt🚀"), KeyboardButton("Stop fuck gt"))
...
def approve_user(message):
    if not is_user_admin(message.from_user.id, message.chat.id):
        bot.send_message(message.chat.id, "*GT KI MAA CHODNE K LIYE ADMIN SE BAT KRO*", parse_mode='Markdown')
        return
